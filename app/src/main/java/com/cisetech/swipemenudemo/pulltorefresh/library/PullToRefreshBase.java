...
import com.cisetech.swipemenudemo.R;
import com.cisetech.swipemenudemo.pulltorefresh.library.internal.FlipLoadingLayout;
import com.cisetech.swipemenudemo.pulltorefresh.library.internal.LoadingLayout;
import com.cisetech.swipemenudemo.pulltorefresh.library.internal.RotateLoadingLayout;
import com.cisetech.swipemenudemo.pulltorefresh.library.internal.Utils;
import com.cisetech.swipemenudemo.pulltorefresh.library.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	@Override
	public final void onRefreshComplete(boolean showComplete) {
		if (isRefreshing()) {
			setState(showComplete,State.RESET);
		}
	}
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					if (mState == State.RELEASE_TO_REFRESH
						...
						setState(false,State.REFRESHING, true);
						return true;
					}
	...
	public final void setRefreshing(boolean doScroll) {
		if (!isRefreshing()) {
			setState(false,State.MANUAL_REFRESHING, doScroll);
		}
	}
	...
	final void setState(boolean flag,State state, final boolean... params) {
		mState = state;
		if (DEBUG) {
	 ...
	 * @param flag
	 */
	protected void onReset(boolean flag) {
		mIsBeingDragged = false;
		mLayoutVisibilityChangesEnabled = true;
		// Always reset both layouts, just in case...
		if(flag){
			mHeaderLayout.reset2();
			mFooterLayout.reset2();
	...
	protected final void setHeaderScroll(int value) {
		...
		switch (getPullToRefreshScrollDirection()) {
			case VERTICAL:
				scrollTo(0, value);
				Log.e("PullToRefresh", "headerValue"+value);
				break;
			case HORIZONTAL:
				scrollTo(value, 0);
				break;
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
				setState(false,State.PULL_TO_REFRESH);
			} else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
				setState(false,State.RELEASE_TO_REFRESH);
			}
		}
	}
